...
- **Demo scripts, not automated tests:** The `scripts/demo-s*.sh` scripts are _demos_, not automated pass/fail tests. They run the program and display output for a human to review. The human decides whether the sprint's work is accepted. Future scripts should follow this pattern: show what to look for, run the program, let the human judge. Run from repo root: `./scripts/demo-s1.sh`
